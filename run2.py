...
class Search():
    ...
    def search_key_word(self):
        ...
        for file in data:
            for pagename in data[file]:
                ...
                info += text_query
                info += table_query
        filter_info = filter_result(info)
        return filter_info
    ...
    def pick_word_from_text(self, text, key_word, text_type, file, pagename):
        ...
        try:
            if text_type == 'table':
                ...
                one_line_text = flatten_text(text)
                belong_content_list = one_line_text.split('.')
            ...
            if line is not None:
                ...
                if self.check_is_in(line):
                    ...
                    if text_type == 'table':
                        ...
                        status = False
                        for content_index,content_sentence in enumerate(belong_content_list):
                            # 需要考虑句号在line的情况 否则无法定位到是属于哪个sentence的
                            if '.' not in line:
                                if line.strip() in content_sentence:
                                    status = True
                                    belong_content = get_display_sentences(content_index,belong_content_list,before=2,after=2)
                                    line_obj.update({'content': belong_content})
                            else:
                                if line.split('.')[0]in content_sentence:
                                    status = True
                                    belong_content = get_display_sentences(content_index,belong_content_list,before=2,after=2)
                                    line_obj.update({'content': belong_content})
    ...
    def ignore_case_is_in(self,key_word,text):
        """
        忽略大小写
        """

        m = re.findall(key_word,text, re.IGNORECASE)
        if m:
            return True
        else:
            return False
...
def replace_word(search_keywords, text, strict=True, english_model=True):
    ...
    if strict:
        ...
        key_word_with_color = "\033[1;31;40m《{}》\033[0m".format(
            search_keywords)
        text = re.sub(key_word, key_word_with_color, text,flags=re.I)
        ...
        if english_model:
            ...
            for element in key_word_list:
                key_word_with_color = "\033[1;31;40m《{}》\033[0m".format(element)
                text = re.sub(element, key_word_with_color, text,flags=re.I)
                # text = text.replace(element,"\033[1;31;40m《{}》\033[0m".format(element) )
        else:
            key_word = search_keywords
            key_word_with_color = text
            for element in key_word:
...
def filter_result(search_result):
    import collections
    filter_result = collections.OrderedDict() 
    for n in search_result:
        content = n['content']
        filter_result.setdefault(content,n)
        filter_result[content].setdefault('line_list',[]).append(n['line_num'])

    filter_list = []
    for n in filter_result:
        filter_list.append(filter_result[n])
    return filter_list
...
def search_word(data, search_keywords, strict, english_model):
    ...
    if result:
        for n in result:
            file = n['file']
            page_name = n['pagename']
            content = n['content']
            line = n['line']
            line_list = n['line_list']
...
def get_display_sentences(current,sentence_list,before=2,after=2):
